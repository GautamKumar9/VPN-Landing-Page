...
function CardPlan({ title, price, features, isSelect }) {
  return (
    ...
    >
      ...
      <div className="mt-8">
        <div className="text-center mb-3 font-bold text-xl">{price} <span className="font-normal">{price=="Free" ? "" : "/ mo"}</span></div>
        <button
          className={`border rounded-full py-1 px-10 border-red-500 font-bold border-2  ${
